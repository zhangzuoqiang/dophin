drop database if exists `mp99`;
create database /*!32312 if not exists*/`mp99` /*!40100 default character set utf8 */;
use `mp99`;
set names utf8;
...
create table `category` (
  ...
  `type` tinyint unsigned not null default 1 COMMENT '分类类型:1-文章分类,2-专题,3-品牌分类,4-链接分类',
...
create table `brand_info` (
  ...
  `url` varchar(50) not null comment '品牌官方网址',
  `cid` smallint unsigned not null default '1' comment '所属分类ID',
  `status` tinyint unsigned not null default 0 comment '状态:-1-删除,0-待审核,1-正常',
  `score` smallint unsigned not null default 60 comment '评分(网站)',
  `user_score` int not null default 0 comment '评分(用户加减分方式)',
  `create_time` int unsigned not null default 0 comment '创建时间',
  primary key  (`id`)
...
-- 品牌介绍内容
-- -----------------------------------------------------
drop table if exists `brand_info_content` ;
create table if not exists `brand_info_content` (
  `id` int unsigned not null auto_increment,
  `content` text not null comment '介绍内容',
  primary key (`id`)
) engine = innodb default charset=utf8 comment = '品牌介绍内容';
