...
function focusRadios(who) {
	...
	what.style.border = "1px dotted #333";
	what.style.margin = "0";
	return false;
}
...
function replaceSelects() {
	...
	for (var q = 0; q < selects.length; q++) {
		...
		optionsDiv.style.top = findPosY(selectArea) + selectAreaHeight - selectAreaOptionsOverlap -15+ "px";
		//get select's options and add to options div
		for (var w = 0; w < selects[q].options.length; w++) {
			var optionHolder = document.createElement("p");
			optionHolder.className="optionsDivVisibleP";
			var optionLink = document.createElement("a");
			optionLink.className="optionsDivVisibleA";
			var optionTxt = document.createTextNode(selects[q].options[w].text);
			optionLink.href = "javascript: selectMe('" + selects[q].id + "'," + w + "," + q + ");";
		...
		//insert options div
		button.onblur = Function("myHideOptions("+q+")"); 	
...
function myHideOptions(q){
		$("#optionsDiv"+q).fadeOut(500);
}
